...
def main():
	...
	testing_error=[]
	training_error=[]
	print("Testing for n=6")
	knn=KNN()
	pca=PCA(n_components=6)
	X=pca.fit_transform(features)
	X_test=np.matmul(test_feature,np.transpose(pca.components_))
	# print("No.of components = "+str(k))
	training_error=findError(X,outputs,X,outputs,"Training",knn,i)
	testing_error=findError(X,outputs,X_test,test_output,"Testing",knn,i)
